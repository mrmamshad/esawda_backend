...
class ListingService
{
    ...
    public function search(Request $request): LengthAwarePaginator
    {
        ...
        if ($sub = $request->query('subcat')) {
            $q->where('sub_category', $sub);
        }
        if ($kw = $request->query('keywords')) {
        ...
        if ($request->filled('range1')) {
            $q->where('price', '>=', (int) $request->query('range1'));
        }
        if ($request->filled('range2')) {
            $q->where('price', '<=', (int) $request->query('range2'));
        }
    ...
    public function promoted(int $limit = 8): Collection
    {
        return Post::active()
            ->where(function ($q) {
                $q->where('featured', '1')
