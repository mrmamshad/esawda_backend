...
<div id="page-content">
    <div class="container">
        <ul class="breadcrumb bcstyle2">
            ...
            <li class="active"><a>{{ __('quickad.resubmited_ads') }}</a></li>
        </ul>
        <a href="{{ $link['POST-AD'] ?? '#' }}" class="postadinner"><span> <i class="fa fa-plus-circle"></i> {{ __('quickad.post_ad') }}</span></a>
        <!--end breadcrumb-->
        <section class="page-title center"><h1>{{ __('quickad.resubmited_ads') }}</h1></section>
        <!--end page-title-->
        <section>
            <div class="row mt40">
                                ...
                                <div class="collapse-box"><h5 class="collapse-title"> {{ __('quickad.my_ads') }} <a class="pull-right" data-toggle="collapse" href="#MyAds"><i class="fa fa-angle-down"></i></a></h5>
                                    ...
                                    <div id="MyAds" class="panel-collapse collapse in">
                                        <ul class="acc-list">
                                            ...
                                            <li class="active"><a href="{{ $link['RESUBMITADS'] ?? '#' }}" class="waves-effect"><i class="fa fa-flag"></i> {{ __('quickad.resubmited_ads') }} <span class="badge">{{ $resubmitads ?? '' }}</span></a></li>
                                        </ul>
                                    </div>
                                </div>
                                <div class="collapse-box">
                ...
                <div class="col-sm-9 page-content">
                    <div class="inner-box"><h1>{{ __('quickad.resubmited_ads') }} </h1>
                            ...
                            <table id="js-table-list" class="table table-striped table-bordered add-manage-table table demo footable-loaded footable" data-filter="#filter" data-filter-text-only="true">
                                ...
                                <tr class="ajax-item-listing" data-item-id="{{ data_get($item ?? [], 'id', '') }}" data-search-term="{{ data_get($item ?? [], 'product_name', '') }}">
                                    ...
                                    <td class="ads-details-td width-58-per">
                                        <div>
                                            <p><strong>{{ data_get($item ?? [], 'product_name', '') }}</strong>
                                            ...
                                            <p><strong> {{ __('quickad.posted_on') }} </strong>: {{ data_get($item ?? [], 'created_at', '') }} </p>
                                            <p><strong>{{ __('quickad.located_in') }}:</strong> {{ data_get($item ?? [], 'location', '') }} </p>
                                        </div>
                                    </td>
                                    <td class="price-td width-16-per">
                                        <div>
                                            <span class="label label-success">{{ __('quickad.resubmited') }}</span>
                                        </div>
                                    </td>
                                    <td class="price-td width-16-per">
                                    ...
                                    <td class="action-td width-10-per">
                                        <div>
                                            <p class="opacity1">
                                            <a class="btn btn-danger btn-rounded btn-xs item-js-delete" href="#" data-ajax-action="deleteResumitAd"><i class=" fa fa-trash-o"></i> {{ __('quickad.delete') }}</a>
                                        </p></div>
                                    </td>
                                </tr>
                                @endforeach
