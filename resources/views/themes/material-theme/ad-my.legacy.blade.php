...
<div id="page-content">
    <div class="container">
        <ul class="breadcrumb bcstyle2">
            ...
            <li class="active"><a>{{ __('quickad.my_ads') }}</a></li>
        </ul>
        <a href="{{ $link['POST-AD'] ?? '#' }}" class="postadinner"><span> <i class="fa fa-plus-circle"></i> {{ __('quickad.post_ad') }}</span></a>
        <!--end breadcrumb-->
        <section class="page-title center"><h1>{{ __('quickad.my_ads_listings') }}</h1></section>
        <!--end page-title-->
        <section>
            <div class="row mt40">
                ...
                <div class="col-sm-9 page-content">
                    <div class="inner-box"><h1>{{ __('quickad.my_ads') }} </h1>
                        ...
                        <div class="table-responsive">
                            ...
                            <table id="js-table-list" class="table table-striped table-bordered add-manage-table table demo footable-loaded footable" data-filter="#filter" data-filter-text-only="true">
                                ...
                                <tr>
                                    ...
                                    <th> {{ __('quickad.status') }}</th>
                                    <th data-type="numeric"> {{ __('quickad.price') }}</th>
                                    <th> {{ __('quickad.option') }}</th>
                                </tr>
                                </thead>
                                <tbody>
                                    @foreach($item ?? [] as $item)
                                    <tr class='ajax-item-listing @if((data_get($item ?? [], 'status', ''))=="hide") opapcityLight @endif' data-item-id="{{ data_get($item ?? [], 'id', '') }}" data-search-term="{{ data_get($item ?? [], 'product_name', '') }}"  >
                                        ...
                                        <td class="ads-details-td width-58-per">
                                            <div>
                                                ...
                                                <p><i class="fa fa-clock-o"></i> <strong> {{ __('quickad.posted_on') }} </strong>: {{ data_get($item ?? [], 'created_at', '') }} </p>
                                                <p><i class="fa fa-calendar-times-o"></i> <strong>{{ __('quickad.expiry_date') }}:</strong> {{ data_get($item ?? [], 'expire_date', '') }} </p>
                                                <p><i class="fa fa-map-marker"></i> <strong>{{ __('quickad.located_in') }}:</strong> {{ data_get($item ?? [], 'location', '') }} </p>
                                        ...
                                        <td class="action-td width-10-per">
                                            <div>
                                                <p class="opacity1">
                                                    <a class="btn btn-info btn-rounded btn-xs" href="{{ $link['EDIT-AD'] ?? '#' }}/{{ data_get($item ?? [], 'id', '') }}" data-ajax-action="deleteMyAd"><i class=" fa fa-pencil"></i> {{ __('quickad.edit') }}</a>
                                                </p>
                                                ...
                                                <p class='opacity1'><a class="btn btn-warning btn-rounded btn-xs item-js-hide" href="#" data-ajax-action="hideItem">
                                                        ...
                                                        @if((data_get($item ?? [], 'hide', ''))=="1") <i class="fa  fa-eye"></i> {{ __('quickad.show') }} @endif
                                                    </a>
                                                </p>
                                                <p class="opacity1">
